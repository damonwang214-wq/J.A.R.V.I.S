...
.trim();
const sir = "Sir";
const name = window.__jarvisName ? `, ${window.__jarvisName}` : "";
const say = (s) => s;
if (/^(hi|hii+|hey|hello|yo|good (morning|afternoon|evening))\b/.test(text)) {
if (isMorning()) return f(window.JARVIS_DATA2.goodMorning, [sir]);
if (text.includes("afternoon")) return f(window.JARVIS_DATA2.goodAfternoon, [sir]);
if (text.includes("evening")) return f(window.JARVIS_DATA2.goodEvening, [sir]);
return f(window.JARVIS_DATA2.greeting, [sir]);
}
if (/(who are you|what are you|your (name|identity)|introduce yourself)/.test(text)) {
return window.JARVIS_DATA.lines.persona;
}
if (/(are you (a )?real|are you alive|do you have feelings|are you sentient)/.test(text)) {
return f(window.JARVIS_DATA2.real, []);
}
if (/(are you (an )?ai|are you (an )?agi|are you intelligent)/.test(text)) {
return f(window.JARVIS_DATA2.ai, []);
}
if (/(how are you|how's it going|how do you do|how r u|how are things)/.test(text)) {
return f(window.JARVIS_DATA2.howAreYou, [sir]);
}
if (/(what time|the time|time is it|current time)/.test(text)) {
const t = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
return f(window.JARVIS_DATA2.timeReply, [t, sir]);
}
...
return f(window.JARVIS_DATA2.dateReply, [d, sir]);
}
if (/(tell me )?(a )?joke|make me laugh|another one/.test(text)) {
return pick(window.JARVIS_DATA.jokes);
}
if (/(tell me )?(a )?fact|interesting fact|fun fact/.test(text)) {
return pick(window.JARVIS_DATA.facts);
}
...
return f(window.JARVIS_DATA2.mathOk, [a, math[2], b, Math.round(r * 1000) / 1000, sir]);
}
}
return f(window.JARVIS_DATA2.mathFail, []);
}
const open = text.match(/open (youtube|google|gmail|github|maps|netflix|spotify|twitter|instagram)/);
if (open) {
const sites = window.JARVIS_DATA.sites;
window.open(sites[open[1]], "_blank");
return f(window.JARVIS_DATA2.open, [open[1], sir]);
}
...
if (!spot) return f(window.JARVIS_DATA2.wNotFound, [sir, place]);
const w = await fetch(
...
const desc = window.JARVIS_DATA.weatherText(c.weathercode);
return f(window.JARVIS_DATA2.wOk, [spot.name, temp, desc]);
} catch {
return f(window.JARVIS_DATA2.wFail, [sir]);
}
}
if (/(tell me )?(a )?story|story time/.test(text)) {
return window.JARVIS_DATA.lines.story;
}
const myName = text.match(/(?:my name is|call me|i am|i'm)\s+([a-z]{2,20})/);
if (myName && !/(ai|robot|jarvis|iron man)/.test(myName[1])) {
window.__jarvisName = myName[1][0].toUpperCase() + myName[1].slice(1);
return f(window.JARVIS_DATA2.nameSaved, [window.__jarvisName]);
}
if (/(what('s| is) my name|do you know my name|remember my name)/.test(text)) {
return window.__jarvisName
? f(window.JARVIS_DATA2.nameKnown, [window.__jarvisName])
: f(window.JARVIS_DATA2.nameUnknown, [sir]);
}
if (/(open the pod bay doors|pod bay)/.test(text)) {
return f(window.JARVIS_DATA2.podBay, [sir]);
}
if (/(i love you|love you|marry me)/.test(text)) {
return f(window.JARVIS_DATA.lines.love, [sir]);
}
if (/(who made you|who built you|who created you|your creator|who's your boss)/.test(text)) {
return f(window.JARVIS_DATA2.maker, [name]);
}
if (/^(help|what can you do|commands|menu|list)/.test(text)) {
return window.JARVIS_DATA.lines.help;
}
if (/(thank you|thanks|thx|cheers|much appreciated)/.test(text)) {
return f(window.JARVIS_DATA2.thanks, [sir]);
}
if (/(bye|goodbye|good night|see you|good night|shutdown|power down|goodnight)/.test(text)) {
return f(window.JARVIS_DATA2.bye, [sir]);
}
return f(window.JARVIS_DATA2.fallback, [sir]);
}
window.jarvisRespond = jarvisRespond;
